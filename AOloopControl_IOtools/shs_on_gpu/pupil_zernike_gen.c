...
#include "pupil_zern_generator/PupilZernikeGenerator_interface.h"
#include <math.h>
...
// stream name of the zernike coefficients
...
{
    {
        CLIARG_IMG,
        ".zernike_coeff",
        "input coefficients, 1D (only first line is used)",
        "zerncoeff", // default
    ...
    },
    {
        CLIARG_ONOFF,
        ".2Doutput",
        "Generate 2D pupil instead of 1D output",
        "0",
...
{
    "zernikeGen",
    "zernike coefficient expansion on pupil",
...
static errno_t streamprocess(PupilZernGenHandle ZGenerator)
{
    DEBUG_TRACE_FSTART();
    
    // Code
    errno_t retVal = PupilZernGen_expand_do(
        ZGenerator,
        *output2D);
...
static errno_t compute_function()
{
    ...
    IMGID coeffimg = mkIMGID_from_name(coeffstream);
    resolveIMGID(&coeffimg, ERRMODE_ABORT);
    ...
    printf("== Constructing zernike generator ...\n");
    ...
    {
        streamprocess(ZGenerator);
    }
    INSERT_STD_PROCINFO_COMPUTEFUNC_END

    // === Cleaning up
    //processinfo_update_output_stream(processinfo, outimg.ID);
    printf("== Deleting Zernike Generator.\n");
    free_PupilZernGen(ZGenerator);
    ZGenerator = NULL;
    // ===
